...
import { hasIdentifier, type DecisionInboxPacket } from "./decisionTypes";
...
function renderView(doc: DecisionInboxPacket | null = FIXTURE) {
  return render(
    <MemoryRouter>
      <DecisionInboxView doc={doc} />
    </MemoryRouter>,
  );
}

describe("DecisionInboxView", () => {
  it("renders the four required fields on a card (title/why/recommendation + urgency)", () => {
    renderView();
    ...
    expect(within(reco).getByTestId("card-recommendation").textContent).toContain("להחיל את השינוי");
    // urgency chip present on each decision card
    expect(screen.getAllByTestId("urgency-chip").length).toBeGreaterThan(0);
  });
  ...
  it("renders the section headline counts", () => {
    renderView();
    expect(screen.getByText("16 פריטים דורשים אותך", { exact: false })).toBeTruthy();
  });
});
