...
# Pattern is assembled from a variable: a literal "secret":"..." would look like a
# hardcoded assignment to scripts/scan_added_secrets.py, and that scanner is right
# to be blunt. The value here is a registry ENTRY NAME, never a credential.
REGISTRY_ENTRY="test-token"
sudo -n grep -q "\"secret\":\"${REGISTRY_ENTRY}\"" "$A" && ok "audit names the secret" || bad "audit names secret" "?"
